...
if (isset($_POST['delete']) && isset($_POST['id'])) {
    ...
    $delete_sql = "DELETE FROM surat_keluar WHERE id = ?";
    $stmt = $conn->prepare($delete_sql);
    $stmt->bind_param('i', $id);

    if ($stmt->execute()) {
        // Redirect to the previous page or any other page after successful deletion
        header('Location: arsipsuratkeluar.php?message=delete_success');
    } else {
        // Handle the error, perhaps redirect with an error message
        header('Location: arsipsuratkeluar.php?message=delete_error');
    }
